...
## 開発プロセス（Step-by-Step）
- **openapi-first**: API の request / response を変更する場合は、まず OpenAPI 側（`api/` 以下）を編集してから実装作業に入ること。直接生成物を編集してはいけない。修正後は `make gen` を実行してコード生成を行い、`go build ./...` でビルド確認を必ず行うこと。
...
- OpenAPI を編集したら必ず `make gen` を実行してコード生成を行ってください。OpenAPI の変更は生成物（`internal/interface/openapi` 以下）に反映される必要があります。生成後は `go build ./...` でビルド確認を必ず行ってください。
- API の request / response を変更する場合は、まず OpenAPI 側（`api/` 以下）を編集してから実装作業に入ってください。直接生成物を編集してはいけません。手順は次の通りです:
    1. `api/` の OpenAPI スキーマを編集
    2. `make gen` を実行して生成ファイルを更新
    3. 必要なハンドラー／変換関数を `internal/interface/` 側で実装・修正
    4. `go build ./...` と `go test ./...` でビルド・テストを確認
- **認証ヘッダーの追加**: エンドポイントに認証（JWT Bearer トークン）が必要な場合も OpenAPI ファースト。実装前に以下を OpenAPI 側で先に定義すること:
    1. `api/OpenAPI.yaml` の `components.securitySchemes` に `bearerAuth`（type: http, scheme: bearer）を定義
    2. 保護対象の各 paths ファイル（`api/paths/` 以下）の該当 operation に `security: [{ bearerAuth: [] }]` を追加
    3. `make gen` を実行して生成ファイルを更新してから実装に入る
...
これによりスキーマと実装の不整合を防ぎ、生成コードの上書きにより作業が無効化されるリスクを避けます。
