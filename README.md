...
※ 現在のジェネレータ実装（`internal/interface/openapi/generate.go` と `internal/interface/handler_generator.go`、および `scripts/openapi-generator-cli.sh`）に基づいています。運用方針（既存ファイルをスキップする）は現在の設定どおりです。ファイル配置やジェネレータの挙動を変更する場合は README を更新してください。
